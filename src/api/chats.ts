 ...
 */
export const createMessage = async (content: FormData, chatId: string) => {
  try {
...
export const markMessageAsSeen = async (messageId: string) => {
  try {
    const res: ResInterface = await api.patch(`/message/seen/${messageId}`);
    return res;
  } catch (error) {
...
export const getChatMessages = async (chatId: string) => {
  try {
    const res: ResInterface = await api.get(`/message/chat/${chatId}`);
    return res;
  } catch (error) {
 ...
 */
export const getUnseenMessages = async (chatId: string) => {
  try {
    const res: ResInterface = await api.get(`/message/unseen/${chatId}`);
    return res;
  } catch (error) {
...
export const deleteMessage = async (messageId: string) => {
  try {
    ...
    const res: ResInterface = await api.delete(`/message/${messageId}`);
    toast.dismiss();
    toast.success(res.message, successStyles);
    return res;
  } catch (error) {
