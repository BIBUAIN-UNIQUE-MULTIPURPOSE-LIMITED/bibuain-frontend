...
const MarketCard = ({
  ...
  <div className={`bg-white rounded-xl p-6 shadow-md hover:shadow-lg transition-all duration-300 ${bgColor}`}>
    <div className="flex justify-between items-center mb-6">
      <div className="flex items-center gap-3">
        <div className="w-[100px] h-12 rounded-lg flex items-center justify-center">
          <img
            src={logo}
            alt={platform}
    ...
    <div className="bg-gray-50 p-5 rounded-xl flex flex-col gap-3 border border-gray-100">
      ...
      <h2 className="text-xl font-bold text-gray-700">
        ₦ {Number(rate2 || 0).toLocaleString()} <span className="text-[14px]">BTC/NGN</span>
      </h2>
    </div>
  </div>
);
