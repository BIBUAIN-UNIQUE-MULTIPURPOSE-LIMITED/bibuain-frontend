...
export const addBank = async (data) => {
    try {
        ...
        const res = await api.post("/banks/add", data);
        toast.dismiss();
...
export const getFreeBanks = async () => {
    try {
        const res = await api.get("/banks/free");
        return res;
    }
    catch (error) {
        handleApiError(error);
...
export const getFundedBanks = async () => {
    try {
        const res = await api.get("/banks/funded");
        return res;
    }
    catch (error) {
        handleApiError(error);
...
export const getSingleBank = async (id) => {
    try {
        const res = await api.get(`/banks/single/${id}`);
        return res;
    }
    catch (error) {
        handleApiError(error);
